...
$(document).ready(function () {
	...
	$("#grid1").jqGrid({
		styleUI : 'Bootstrap',
		caption: "Новинки в нашем ассортименте",
		mtype: "GET",
		url: "/engine/jqgrid3?action=good_newlist_b2b&sid=5&group=-1&f1=Article&f2=Name&f3=Brand&f4=PriceBase&f5=Price&f6=PriceRR&f7=Margin&f8=Unit_in_pack&f9=FreeBalance&f10=FreeBalance23&f11=Qty&f12=OPT_ID",
		responsive: true,
		scroll: true, 
		height: 462, // если виртуальная подгрузка страниц
		datatype: "json",
	    colModel: [
			{label:'Артикул',		name:'g.Article',		index:'g.Article',	width: 120, sorttype: "text",	search: true,
				searchoptions: { 
			...
			{label:'Название',		name:'g.Name',		index:'g.Name',		width: 220, sorttype: "text",	search: true, hidedlg: false,
				searchoptions: { 
					dataInit: function (element) {
						$(element).attr("autocomplete","off").typeahead({ 
							autoSelect: false, items:'20', minLength:3,	appendTo : "body",
			...
			{label:'Бренд',		name:'g.Brand',		index:'g.Brand',		width: 120, sorttype: "text", align: "center", 	search: true, hidedlg: false,
				searchoptions: { 
					dataInit: function (element) {
						$(element).attr("autocomplete","off").typeahead({ 
							autoSelect: false, items:'20', minLength:3,	appendTo : "body",
		...
		//	if(!fs) {fs = true; filter_restore("#grid1");}
		}
	});
	...
	$("#grid1").jqGrid('filterToolbar', { autosearch: true, searchOnEnter: true,
		beforeSearch: function () {
			...
			//console.log("gs_FreeBalance:",$("#gs_FreeBalance"));
